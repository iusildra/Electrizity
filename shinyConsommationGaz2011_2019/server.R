...
shinyServer(function(input, output) {
  ...
  output$departementMap <- renderLeaflet({
    ...
    data1119 <- data1119[complete.cases(data1119), ]
    departement1119<- aggregate(consommation_MWh ~ departement+habitant_departement, data1119, sum)

    data1119$centroidLng <- as.numeric(gsub("^.*?,","",data1119$centroid))
    data1119$centroidLat <- as.numeric(gsub(",.*","",data1119$centroid))

    centroidLng <- aggregate(centroidLng ~ departement, data1119, mean)
    centroidLat <- aggregate(centroidLat ~ departement, data1119, mean)

    departement1119 <- merge(departement1119, centroidLng, by= "departement")
    departement1119 <- merge(departement1119, centroidLat, by= "departement")


    leaflet(departement1119) %>%
      addTiles() %>%
      addCircles(lng=~departement1119$centroidLng, lat=~departement1119$centroidLat, stroke = FALSE, fillOpacity= 0.7,
